...
def _setup_large_toon_data_file():
    ...
    if not os.path.exists(LARGE_TOON_FILE):
        try:
            ...
            print(f"Generated large TOON file: {LARGE_TOON_FILE} ({os.path.getsize(LARGE_TOON_FILE) / (1024*1024):.2f} MB) for memory profiling.")
...
class TestMemoryPerformance:
    ...
    def test_decode_large_file_memory_usage_rust_decoder(self, tmp_path):
        ...
        assert result.returncode == 0, f"Memory profiling script failed with exit code {result.returncode}:\n{result.stderr}"
        assert peak_memory_mb > 0.0, f"Memory profiling did not capture any memory usage. Log content:\n{log_content}"
